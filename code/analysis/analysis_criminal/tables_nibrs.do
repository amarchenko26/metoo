...
if `run_did' == 1 {
	...
	foreach y of local outcome_vars {
		...
		reghdfe ``y'' treat, absorb(time_state) vce(robust)
		eststo s`i'
	...
	#delimit ;	
	estout a1 s1 a2 s2 a3 s3 using "$tables/did_nibrs.tex", style(tex) replace
		...
		stats(fetime fetime_c N r2, 
			label("Time FE" "Time $\times$ State FE" `"N"' `" \(R^{2}\)"') fmt(3 3 %9.0fc 3))
...
if `run_summary' == 1 {
	...
	esttab using "$tables/summary_nibrs.tex", replace ///
...
if `run_balance' == 1 {
    ...
    balancetable_program `balance', using("$tables/balance_nibrs.tex") ctitles("Before" "After" "Diff" "p-value") wide(mean diff pval) by(post) errors(robust)

    balancetable_program `balance', sample(sex_cases == 1) using("$tables/balance_sex_nibrs.tex") ctitles("Before" "After" "Diff" "p-value") wide(mean diff pval) by(post) errors(robust)

    // Filed pre-covid
    balancetable_program `balance', sample(ym < 721) using("$tables/balance_covid_nibrs.tex") ctitles("Before" "After" "Diff" "p-value") wide(mean diff pval) by(post) errors(robust)
