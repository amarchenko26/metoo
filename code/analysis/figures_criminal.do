...
if `event' == 1 {
	...
	foreach horizon in `horizons' {
		foreach y in `outcomes' {
			...
			graph export "$figures/honestdid_`y'_`horizon'_criminal.png", replace
			...
			graph export "$figures/eventstudy_`y'_`horizon'_criminal.png", replace 
...
if `timeseries' == 1 {
    ...
    graph export "$figures/timeseries_criminal.png", replace
    restore
...
// Probability of winning over time
	...
	collapse (mean) mean_clear = clearance, by(ym sex_cases)
		twoway ///
			...
			xline(693, lpattern(solid))
	graph export "$figures/timeseries_clearprob_criminal.png", replace 	
	restore	
}
...
if `diff' == 1 {
	...
	twoway ///
		...
		xline(693)
	graph export "$figures/timeseries2_criminal.png", replace 	
	...
	twoway ///
		...
		xtitle("Date filed", size(medium)) ytitle("Number of cases", size(medium)) ///
		xline(693)
	graph export "$figures/timeseries_nocovid_criminal.png", replace 	
restore	
}
